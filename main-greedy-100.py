...
def greedy_feature_select(data_holder, dev_data):
    ...
    while len(s[1]) < 100:
        ...
        if best_t[0] >= s[0]:
            ...
            cv_number = cross_validation(5, s[1],data_holder)
            results.append(cv_number)
            print len(s[1])
        else:
            return results
    return results
...
def cross_validation(num_batches, features, data_holder):
    ...
    correct = 0.0
    total = 0.0
    old_writers = data_holder.writers
    for test in range(num_batches):
        ...
        for writer in test_data:
            for doc in test_data[writer]:
                ...
                if label == writer:
                    correct += 1
    data_holder.set_writers(old_writers)
...
def main():
    ...
    for i in range(3):
        ...
        if sys.argv[1] == 'naive':
            ...
            print greedy_feature_select(data_holder, dev_data)
            return
